...
class Environment(BaseAviary):
    ...
    def _actionSpace(self):
        ...
        action_lower = np.array([0.0, 0.0, 0.0, 0.0])
    ...
    def _addObstacles(self):
        ...
        if self.obstacle_type == 0:
            radius = 0.2
            height = 10
            columnID = pb.createCollisionShape(
                shapeType=pb.GEOM_CYLINDER, radius=radius, height=height
            )
            for i in range(-5, 6, 2):
                for j in range(-5, 6, 2):
                    if i == 0 and j == 0:
                        continue
                    column = pb.createMultiBody(
                        baseMass=0,
                        baseCollisionShapeIndex=columnID,
                        basePosition=[i, j, 0.5],
                    )
                    self.columnIDs.append(column)
    ...
    def createTarget(self):
        ...
        new_x = np.clip(drone_pos[0] + np.random.uniform(-3, 3), -5, 5)
        new_y = np.clip(drone_pos[1] + np.random.uniform(-3, 3), -5, 5)
        new_z = np.clip(drone_pos[2] + np.random.uniform(-3, 3), 0.25, 5)
...
class RepeatAction(gym.Wrapper):
    ...
    def step(self, action):
        ...
        for _ in range(self.n_repeat):
            ...
            if done:
                break
        return obs, total_reward, done, 42
